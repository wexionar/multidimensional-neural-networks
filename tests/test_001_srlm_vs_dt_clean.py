...
def run_test_001():
    ...
    print("🧪 TEST 001: STRUCTURAL PURITY (CLEAN DATA)")
    print("Config: SRLM-nD Epsilon 0.08 | PURITY MODE")
    print("-" * 50)
    ...
    t_tree = time.time() - start_tree
    mae_tree = np.mean(np.abs(Y_norm - model_tree.predict(X)))

    # --- B) SRLM-nD ---
    start_srlm = time.time()
    origin = LuminOrigin(epsilon_val=0.08, epsilon_type=1, mode_type=2) 
    for point in data_fusion: origin.ingest(point)
    t_srlm = time.time() - start_srlm
    ...
    # RESULTS DISPLAY
    results = {
        "Metric": ["MAE (Accuracy)", "Logic Units", "Execution Time"],
        "Decision Tree": [f"{mae_tree:.6f}", f"{model_tree.tree_.node_count} nodes", f"{t_tree:.4f}s"],
        "SRLM-nD": [f"{mae_srlm:.6f}", f"{len(origin.master_sectors)} sectors", f"{t_srlm:.4f}s"]
    }
    print(pd.DataFrame(results).to_string(index=False))
    print("-" * 50)
    
    ratio = model_tree.tree_.node_count / len(origin.master_sectors)
    print(f"💡 CONCLUSION: SRLM-nD is {ratio:.2f}x more synthetic than the Decision Tree.")
