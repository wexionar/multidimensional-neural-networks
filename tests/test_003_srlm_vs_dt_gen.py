...
class LuminResolution:
    ...
    def resolve(self, X_input):
        ...
        # Margin of tolerance to find the appropriate sector
        inside = np.all((X[:, np.newaxis, :] >= self.mins - 0.05) & (X[:, np.newaxis, :] <= self.maxs + 0.05), axis=2)
...
# =============================================================
# TEST 003: GENERALIZATION POWER (NOISY TRAIN VS CLEAN VAL)
# =============================================================
def run_test_003():
    np.random.seed(42)
    
    # 1. Training Data (WITH NOISE)
    X_train = np.random.uniform(0, 1, (10000, 5))
    Y_train = 10 * np.sin(np.pi * X_train[:,0] * X_train[:,1]) + 20 * (X_train[:,2] - 0.5)**2 + 10 * X_train[:,3] + 5 * X_train[:,4]
    Y_train += np.random.normal(0, 0.5, 10000)
    
    # 2. Validation Data (CLEAN - The Ground Truth)
    X_val = np.random.uniform(0, 1, (2000, 5))
    Y_val = 10 * np.sin(np.pi * X_val[:,0] * X_val[:,1]) + 20 * (X_val[:,2] - 0.5)**2 + 10 * X_val[:,3] + 5 * X_val[:,4]
    ...
    data_fusion = np.hstack([X_train, Y_train_norm.reshape(-1, 1)])

    print("🧪 TEST 003: GENERALIZATION (NOISY TRAIN VS CLEAN VAL)")
    print("Config: SRLM-nD Epsilon 0.10 | PURITY MODE")
    print("-" * 50)

    # --- A) DECISION TREE ---
    model_tree = DecisionTreeRegressor(max_depth=12, random_state=42)
    model_tree.fit(X_train, Y_train_norm)
    y_pred_tree = model_tree.predict(X_val)
    mae_tree = np.mean(np.abs(Y_val_norm - y_pred_tree))

    # --- B) SRLM-nD ---
    origin = LuminOrigin(epsilon_val=0.10, epsilon_type=1, mode_type=2) 
    for point in data_fusion: origin.ingest(point)
    res = LuminResolution(origin.master_sectors)
    y_pred_srlm = res.resolve(X_val)
    
    mask = ~np.isnan(y_pred_srlm)
    mae_srlm = np.mean(np.abs(Y_val_norm[mask] - y_pred_srlm[mask]))
    
    # RESULTS DISPLAY
    results = {
        "Metric": ["MAE (on clean data)", "Logic Units"],
    ...
    print(f"💡 CONCLUSION: SRLM-nD uses {ratio:.2f}x fewer units to predict clean reality.")
